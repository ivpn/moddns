...
func (c *RedisCache) AddPASession(ctx context.Context, session *model.PASession, expiresIn time.Duration) error {
	...
	if err != nil {
		log.Err(err).Msg("Cache: failed to marshal PA session")
		return err
	}
	if err := c.client.Set(ctx, key, string(data), expiresIn).Err(); err != nil {
		log.Err(err).Msg("Cache: failed to add PA session")
		return err
	}
	log.Info().Dur("expires_in", expiresIn).Msg("Cache: added PA session")
	return nil
}
...
func (c *RedisCache) GetPASession(ctx context.Context, sessionID string) (*model.PASession, error) {
	...
	if err != nil {
		log.Err(err).Msg("Cache: failed to get PA session")
		return nil, err
	}
	var session model.PASession
	if err := json.Unmarshal([]byte(val), &session); err != nil {
		log.Err(err).Msg("Cache: failed to unmarshal PA session")
		return nil, err
	}
	return &session, nil
}
...
func (c *RedisCache) RemovePASession(ctx context.Context, sessionID string) error {
	...
	if err := c.client.Del(ctx, key).Err(); err != nil {
		log.Err(err).Msg("Cache: failed to remove PA session")
		return err
	}
	log.Info().Msg("Cache: removed PA session")
	return nil
}
