...
func TestNotifyInactive_FreshTier1(t *testing.T) {
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{}, nil).Once()

	// Find step: returns our Tier 1 sub.
	subRepo.On("FindInactiveUnnotified", mock.Anything).Return([]model.Subscription{sub}, nil).Once()
	...
	mailer.On("SendInactiveEmail", mock.Anything, "victim@example.com").Return(nil).Once()
	subRepo.On("SetInactiveNotified", mock.Anything, []uuid.UUID{sub.ID}, true).Return(nil).Once()

	NotifyInactiveSubscriptions(subRepo, accountRepo, profileRepo, profileCache, mailer)
}

// TestNotifyInactive_PostFilterSkipsPaidActive proves the loose Mongo
...
func TestNotifyInactive_PostFilterSkipsPaidActive(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{}, nil).Once()
	subRepo.On("FindInactiveUnnotified", mock.Anything).Return([]model.Subscription{leakedActive}, nil).Once()

	// Deliberately register no expectations on accountRepo, profileRepo, cache,
	// mailer, or SetInactiveNotified — if the cron calls any of them, the
	// mock will fail the test.

	NotifyInactiveSubscriptions(subRepo, accountRepo, profileRepo, profileCache, mailer)
}

// TestNotifyInactive_ReArmClearsFlagWhenNoLongerPD covers the recovery
...
func TestNotifyInactive_ReArmClearsFlagWhenNoLongerPD(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{recovered}, nil).Once()
	// Re-arm fires: GetStatus() != PD → bulk-clear the flag for this sub's ID.
	subRepo.On("SetInactiveNotified", mock.Anything, []uuid.UUID{recovered.ID}, false).Return(nil).Once()

	// Then find step finds no candidates.
	subRepo.On("FindInactiveUnnotified", mock.Anything).Return([]model.Subscription{}, nil).Once()

	NotifyInactiveSubscriptions(subRepo, accountRepo, profileRepo, profileCache, mailer)
}

// TestNotifyInactive_ReArmKeepsFlagWhenInLA verifies the one-shot
...
func TestNotifyInactive_ReArmKeepsFlagWhenInLA(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{partial}, nil).Once()
	// No SetInactiveNotified call expected — re-arm must NOT clear the flag.
...
// TestNotifyInactive_ReArmKeepsFlagWhenStillPD covers the idempotency
// case: a flagged Tier 1 sub stays flagged tick after tick (no re-email).
func TestNotifyInactive_ReArmKeepsFlagWhenStillPD(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{stillPD}, nil).Once()
	// No SetInactiveNotified call — re-arm finds nothing to clear.
...
// TestNotifyInactive_EmailFailureLeavesUnnotified verifies the retry
// contract: if SendInactiveEmail fails, the sub must not be marked
// notified so the cron retries on the next tick.
//
// specRef: subscription-lifecycle-enforcement.md E6 (analogous LA contract)
func TestNotifyInactive_EmailFailureLeavesUnnotified(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	mailer.On("SendInactiveEmail", mock.Anything, "fail@example.com").Return(errors.New("smtp boom")).Once()

	// DNS cutoff still happened (no profiles to delete in this case), but
	// SetInactiveNotified must NOT be called because the email failed.
	// We register no expectation on SetInactiveNotified; if it fires,
	// the strict mock fails.

	NotifyInactiveSubscriptions(subRepo, accountRepo, profileRepo, profileCache, mailer)
}

// TestNotifyInactive_SkipsRetired proves a retired (signup-reset) sub —
...
func TestNotifyInactive_SkipsRetired(t *testing.T) {
	subRepo := mocks.NewSubscriptionRepository(t)
	accountRepo := mocks.NewAccountRepository(t)
	profileRepo := mocks.NewProfileRepository(t)
	profileCache := mocks.NewCachecache(t)
	...
	subRepo.On("FindWithInactiveNotified", mock.Anything).Return([]model.Subscription{}, nil).Once()
	subRepo.On("FindInactiveUnnotified", mock.Anything).Return([]model.Subscription{retired}, nil).Once()
