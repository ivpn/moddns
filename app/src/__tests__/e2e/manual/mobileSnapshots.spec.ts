...
// Trigger ONLY via: npm run snapshots:mobile
// playwright.config.ts ignores this directory unless MOBILE_SNAPSHOTS=1 is set.
...
test.describe('Manual mobile full-page snapshots', { tag: '@mobile' }, () => {
  for (const route of ROUTES) {
    test(`snapshot: ${route}`, async ({ page }) => {
      ...
      const device = project === 'chromium-mobile-dark' ? 'android' : 'ios';
